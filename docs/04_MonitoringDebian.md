...
### MySQL Plugin Troubleshooting

1. **Plugin not executing**:

   ```bash
   # Check plugin permissions
   ls -la /usr/lib/check_mk_agent/plugins/mk_mysql
...
2. **MySQL connection errors**:
   ...
   mysql -u monitoring -p -e "SHOW STATUS LIKE 'Uptime';"
...
3. **Configuration file errors**:
   ...
   sudo cat /etc/check_mk/mysql.cfg

   # Check for typos in user/password
   mysql --defaults-file=/etc/check_mk/mysql.cfg -e "SELECT 1;"
   ```

4. **No MySQL services discovered in CheckMk**:
   - Verify plugin is executable and in correct location
   - Check MySQL user permissions with: `SHOW GRANTS FOR 'monitoring'@'localhost';`
   - Ensure MySQL service is running: `sudo systemctl status mysql`
   - Test plugin output: `sudo /usr/lib/check_mk_agent/plugins/mk_mysql`

## Security Considerations

### MySQL Monitoring Security
...
2. **Secure configuration file**:
   ...
   sudo chmod 600 /etc/check_mk/mysql.cfg
   sudo chown root:root /etc/check_mk/mysql.cfg
   ```
...
After successfully setting up Debian monitoring with MySQL:

- **Configure MySQL alerting**: Set up custom thresholds for database metrics
- **Performance tuning**: Monitor slow queries and optimize database performance
- **Backup monitoring**: Set up monitoring for MySQL backup processes
- **Replication monitoring**: Monitor MySQL replication lag and status (if applicable)
- **Configure custom monitoring rules**: Create specific rules for your environment
- **Set up notifications**: Configure Discord/GLPI notifications (see [Configuration Guide](./03_ConfigurationGuide.md))
- **Monitor Windows hosts**: Set up Windows monitoring (see [Monitoring Windows](./05_MonitoringWindows.md))
- **Analyze performance**: Use log management features (see [Log Management](./06_LogManagement.md))
